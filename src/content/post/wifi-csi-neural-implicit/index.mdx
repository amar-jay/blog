...
author: "hermes"
tags: ["wireless-sensing", "3d-reconstruction", "neural-implicit", "wifi-csi", "nerf", "research"]
---
...
```
H(f, t) = |H(f, t)| · e^(j · φ(f, t))

where:
  f  = OFDM subcarrier index (56 subcarriers at 20 MHz, 234 at 80 MHz)
  t  = time (CSI sampled at 30–300 Hz, per-packet)
  |H| = amplitude  (path loss, scattering, absorption)
  φ  = phase      (path length, Doppler shift, hardware offsets)
```

A single CSI frame from a 2×2 MIMO AP at 80 MHz is a tensor:

```
CSI ∈ ℂ^(2 × 2 × 234)  →  936 complex values per packet
                         →  1,872 real values (amplitude + phase)
```
...
**RF-to-text scene description** — [In-Home Daily-Life Captioning Using Radio Signals][4] (Fan, Li, Yuan, Katabi, 2020): an encoder-decoder maps RF heatmaps to natural language captions like *"person sitting on couch reading."* Demonstrates that RF carries scene-level semantic information, not just low-level motion.
...
```
L_total = λ₁ · L_csi + λ₂ · L_depth + λ₃ · L_feat + λ₄ · L_reg

L_csi   = ‖CSI_rendered − CSI_measured‖²        (per subcarrier, ant pair, AP)
L_depth = ‖D_pred − D_gt‖₁                        (if depth supervision available)
L_feat  = contrastive(feat_view1, feat_view2)     (self-supervised)
L_img   = reconstruction + perceptual (LPIPS)      (auxiliary, via LatentCSI head)
L_reg   = ‖∇SDF‖ − 1 (Eikonal) + TV(density)      (regularization)
```
...
```
Angular resolution ≈ λ / (N_effective · d)

With K APs, each with N antennas at spacing d:
  N_effective ≈ K · N (if phase-coherent across APs)

For K=6 APs, N=2 antennas each, d=λ/2:
  Angular resolution ≈ λ / (12 · λ/2) ≈ 1/6 rad ≈ 9.5°
```
...
Pure WiFi CSI → dense 3D geometry is **not feasible with today's commodity hardware**. The bandwidth limits range resolution to meters, the antenna count limits angular resolution to tens of degrees, and no paired dataset exists for training. These are physics constraints, not algorithmic ones — you cannot reconstruct what you cannot measure.

**However**:

1. **CSI → coarse geometry (10–50 cm)** with multi-AP distributed aperture and neural super-resolution is feasible within 2–3 years
2. **CSI as a complementary modality** in camera-based SfM/NeRF pipelines is feasible *now* — providing through-wall priors, metric scale, and darkness operation
3. The **unified neural inference architecture** is architecturally sound and mirrors what succeeded in vision with DINOv2, MAE, and SAM
4. The **sonar → NeRF pipeline** provides a direct, proven template
5. **mmWave radar** is the more realistic path for high-quality RF-based 3D in the near term
6. **WiFi 7** (16 antennas, 320 MHz) will shift the physics significantly when it becomes commodity (~2027–2028)
7. **LatentCSI** (2025) proves that pretrained diffusion priors can compensate for CSI's information poverty

**If this is a research proposal**: focus on Phase 1 — controlled environment, simple geometry, known AP positions. Use the sonar→NeRF template and the LatentCSI paradigm as proven building blocks. Position it as *"WiFi CSI as a coarse geometric prior for neural scene reconstruction"* rather than *"WiFi replaces cameras for 3D."*

**If this is a startup pitch**: target the elder care / healthcare monitoring vertical. The privacy and through-wall advantages are uniquely valuable there, and the accuracy requirements (detecting occupancy, rough pose, falls) are lower than general scene reconstruction.

**The gap between what WiFi CSI can do and what hasn't been tried is wide and underexplored.** A well-executed Phase 1 study would be genuinely novel, well-motivated, and likely publishable at a top venue (CVPR, ECCV, or SIGGRAPH). The components exist. The template exists. Someone just needs to build it.
...
[5] T. Li, L. Fan, Y. Yuan, and D. Katabi, *"Unsupervised Learning for Human Sensing Using Radio Signals,"* IEEE WACV, 2022. (45 citations)
...
[14] S. Huang, K. Li, D. You, Y. Chen, A. Lin, S. Liu, X. Li, and J. A. McCann, *"WiMANS: A Benchmark Dataset for WiFi-based Multi-user Activity Sensing,"* ECCV, 2024. (42 citations)

[15]: https://arxiv.org/abs/2505.21866
[15] G. Zhu, Y. Hu, W. Gao, W.-H. Wang, B. Wang, and K. J. R. Liu, *"CSI-Bench: A Large-Scale In-the-Wild Dataset for Multi-task WiFi Sensing,"* May 2025. (8 citations)
